...
This project provides a command-line tool to convert EPUB ebooks into audiobooks. It now supports the [Microsoft Azure Text-to-Speech API](https://learn.microsoft.com/en-us/azure/cognitive-services/speech-service/rest-text-to-speech) (alternativly [EdgeTTS](https://github.com/rany2/edge-tts)), the [OpenAI Text-to-Speech API](https://platform.openai.com/docs/guides/text-to-speech), and [Inworld TTS](https://docs.inworld.ai/api-reference/ttsAPI/texttospeech/synthesize-speech) to generate the audio for each chapter in the ebook. The output audio files are optimized for use with [Audiobookshelf](https://github.com/advplyr/audiobookshelf).
...
- For using *Inworld TTS*, the Inworld Basic (Base64) API credential is required.
...
4. Set the following environment variables with your Azure Text-to-Speech API credentials, your OpenAI API key, or your Inworld TTS basic credential:
    ...
    export INWORLD_API_KEY=<your_inworld_basic_base64_credential> # for Inworld
    export INWORLD_VOICE_ID=<your_inworld_voice_id> # optional default voiceId
    export INWORLD_MODEL_ID=<your_inworld_model_id> # optional default modelId
    export INWORLD_TTS_BASE_URL=<custom_inworld_base_url> # optional endpoint override
    ```
...
export INWORLD_API_KEY=<your_inworld_basic_base64_credential>  # For Inworld TTS
export INWORLD_VOICE_ID=<your_inworld_voice_id> # Optional default voiceId
export INWORLD_MODEL_ID=<your_inworld_model_id> # Optional default modelId
export INWORLD_TTS_BASE_URL=<custom_inworld_base_url> # Optional endpoint override
```
...
- **TTS Provider Selection**: Easy switching between Azure, OpenAI, Inworld, Edge, and Piper TTS with provider-specific options
...
3. **Configure provider-specific settings**:
   ...
   - **Inworld**: Set voice ID, model ID, and output format
...
```bash
usage: main.py [-h] [--tts {azure,openai,inworld,edge,piper}]
...
options:
  ...
  --tts {azure,openai,inworld,edge,piper}
                        Choose TTS provider (default: azure). azure: Azure
                        Cognitive Services, openai: OpenAI TTS API, inworld:
                        Inworld REST TTS. When using azure, environment
                        variables MS_TTS_KEY and MS_TTS_REGION must be set.
                        When using openai, environment variable
                        OPENAI_API_KEY must be set. When using inworld,
                        environment variable INWORLD_API_KEY must be set
                        with the Basic (Base64) credential.
...
For Inworld, you can run:

```bash
docker run -i -t --rm -v ./:/app -e INWORLD_API_KEY=$INWORLD_API_KEY -e INWORLD_VOICE_ID=$INWORLD_VOICE_ID -e INWORLD_MODEL_ID=$INWORLD_MODEL_ID ghcr.io/p0n1/epub_to_audiobook your_book.epub audiobook_output --tts inworld
```

Replace `$MS_TTS_KEY` and `$MS_TTS_REGION` with your Azure Text-to-Speech API credentials. Replace `$OPENAI_API_KEY` with your OpenAI API key. Replace `$INWORLD_API_KEY` with your Inworld Basic credential. Replace `your_book.epub` with the name of the input EPUB file, and `audiobook_output` with the name of the directory where you want to save the output files.
...
For Inworld TTS, you can specify the voice ID, model ID, and output format using `--voice_name`, `--model_name`, and `--output_format`. Inworld uses Basic auth, so set `INWORLD_API_KEY` to the Basic (Base64) credential before running the tool. `voice_name` maps to `voiceId`, `model_name` maps to `modelId`, and this provider chunks text at 2,000 characters per request. Pricing estimates use Inworld’s published billing rates: `inworld-tts-1.5-mini` is estimated at `$5/1M chars` and `inworld-tts-1.5-max` at `$10/1M chars`.
